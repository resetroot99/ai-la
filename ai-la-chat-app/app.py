...
def build():
    ...
    def generate():
        ...
        try:
            ...
            yield f"data: {json.dumps({'type': 'log', 'message': f'Decisions made with {decisions[\"confidence\"]*100:.1f}% confidence'})}\n\n"
            ...
            if not result['success']:
                yield f"data: {json.dumps({'type': 'error', 'message': f'Generation failed: {result.get(\"error\")}'})}\n\n"
                return
            
            yield f"data: {json.dumps({'type': 'log', 'message': f'Application generated at {result[\"path\"]}'})}\n\n"
            yield f"data: {json.dumps({'type': 'log', 'message': f'Files created: {len(result.get(\"files\", []))}'})}\n\n"
            ...
            if evolution['evolved']:
                yield f"data: {json.dumps({'type': 'log', 'message': f'AI evolved to generation {evolution[\"generation\"]}'})}\n\n"
            else:
                yield f"data: {json.dumps({'type': 'log', 'message': 'AI is already optimal'})}\n\n"
            
            # Complete
            yield f"data: {json.dumps({'type': 'complete', 'result': {'path': str(result['path']), 'project': result['spec']['name']}})}\n\n"
